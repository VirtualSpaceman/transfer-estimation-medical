...
def gaussian_log_expected_empirical_prediction(features: np.ndarray, labels: np.ndarray):
    r"""
    Log Expected Empirical Prediction in `LEEP: A New Measure to
    Evaluate Transferability of Learned Representations (ICML 2020)
    <http://proceedings.mlr.press/v119/nguyen20b/nguyen20b.pdf>`_.
    ...
    features_pca = PCA(n_components=0.8, random_state=42).fit_transform(features)

    #then calculate gmm as density estimator and calculate leep
    gmm = GaussianMixture(n_components=5*num_classes, random_state=42).fit(features_pca)
    gmm_predictions = gmm.predict_proba(features_pca)
